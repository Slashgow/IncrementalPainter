		...
		public static Color GetPixel(RenderTexture renderTexture, Vector2 uv, bool mipMaps = false)
		{
			if (renderTexture != null)
			{
				if (tempReadTexture == null)
				{
					...
					var tempRenderTexture = RenderTexture.GetTemporary(2, 2, 0, RenderTextureFormat.ARGB32);

					Graphics.Blit(tempReadTexture, tempRenderTexture);

					CwHelper.BeginActive(tempRenderTexture);
						tempReadTexture.ReadPixels(new Rect(0, 0, 1, 1), 0, 0);
					CwHelper.EndActive();
