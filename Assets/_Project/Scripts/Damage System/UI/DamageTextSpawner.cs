...
public class DamageTextSpawner : MonoBehaviour
{
    ...
    private void OnEnable() => SimpleDamageor.OnAnyDamageorAttack += HandleDamageorAttack;
    private void OnDisable() => SimpleDamageor.OnAnyDamageorAttack -= HandleDamageorAttack;
    ...
    private void SpawnDamageText(float damageAmount, Vector3 worldPosition, bool isCritical)
    {
        ...
        if (isCritical)
            damageText.InitializeCritical(damageAmount, criticalHitColor, poolingSystem);
        else
            damageText.Initialize(damageAmount, normalHitColor, poolingSystem);
    }
}